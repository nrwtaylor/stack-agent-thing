...
class Sunglasses 
{
    ...
    function __construct(Thing $thing, $agent_input = null)
    {
       ...
       $this->keywords = array('weather', 'sunglasses', 'sunny', 'bright');

        $this->current_time = $this->thing->time();

        $this->variables_agent = new Variables($this->thing, "variables " . "sunglasses" . " " . $this->from);
...
//var_dump($this->weather->forecast_conditions);
        ...
        if ($sunny_current)  {$current_message = "sunny";}
        //if ((is_numeric($this->pop_current)) and ($this->pop_current != 0))  {$current_message .= " " . $this->pop_current . "%";}
        ...
        if ($sunny_forecast)  {$forecast_message = "sunny";}
...
//var_dump($this->pop_forecast);
        ...
        // % pop now
        if (($this->pop_current == null) and ($this->pop_forecast == null)) {$message = "Up to you.";}
        if (($this->pop_current == null) and ($this->pop_forecast >= 30)) {$message = "Up to you.";}
        if (($this->pop_current == null) and ($this->pop_forecast >= 40)) {$message = "Up to you.";}


        // 30% pop now
        if (($this->pop_current >= 30) and ($this->pop_forecast == null)) {$message = "Maybe.";}
        if (($this->pop_current >= 30) and ($this->pop_forecast >= 30)) {$message = "Maybe.";}
        if (($this->pop_current >= 30) and ($this->pop_forecast >= 40)) {$message = "Maybe.";}

        // 40% pop now
        if (($this->pop_current >= 40) and ($this->pop_forecast == null)) {$message = "Possibly.";}
        if (($this->pop_current >= 40) and ($this->pop_forecast >= 30)) {$message = "Possibly.";}
        if (($this->pop_current >= 40) and ($this->pop_forecast >= 40)) {$message = "Possibly.";}

        // 60% pop now
        if (($this->pop_current >= 60) and ($this->pop_forecast == null)) {$message = "It's probably raining. But clearing.";}
        if (($this->pop_current >= 60) and ($this->pop_forecast >= 30)) {$message = "It's probably going to rain. Or is.";}
        if (($this->pop_current >= 60) and ($this->pop_forecast >= 40)) {$message = "It's probably already raining.";}
