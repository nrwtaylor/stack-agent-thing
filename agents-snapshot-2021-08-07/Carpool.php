...
class Carpool extends Agent
{
    ...
    function init() {
        ...
        $this->thing_report['info'] = 'Provides a web link to share with the status of the car pool.';
    ...
    public function set($requested_state = null)
    {
        ...
        if ($requested_state == null) {return true;}
    ...
    {
// devstack not picking up quantity yet.
        $this->quantity = new Quantity($this->thing, "quantity");
//        if ((!isset($this->quantity->quantity)) or ($this->quantity->quantity == null)) {$this->quantity->quantity = "X";}
...
$quantity = "X";
if (isset($this->quantity->quantity)) {$quantity = $this->quantity->quantity;}

        $web .= "The driver says there are ".  $quantity . " seats available.<br>";

        $web .= "The driver is showing a ". $this->flag->state . " flag.<br><br>";

$destination_name = "NOT SET";
if (isset($this->destination->destination_name)) {$destination_name = $this->destination->destination_name;}
...
if (isset($this->flag->html_image)) {$flag_html_image = $this->flag->html_image;}
...
if (isset($this->clocktime->refreshed_at)) {$clock_time_refreshed_at = strtotime($this->clocktime->refreshed_at);}
