...
class Wordpress extends Agent
{
    ...
    public function init()
    {
        ...
        if (isset($this->thing->container['api']['wordpress']['user_id'])) {
            $this->user_id =
                $this->thing->container['api']['wordpress']['user_id'];
        }
        ...
        $this->node_list = ["wordpress" => ["wordpress"]];
    }
...
*/
    ...
    public function readSubject()
    {
        ...
        if (
            ($pos = strpos(strtolower($this->input), "wordpress is")) !== false
        ) {
            $whatIWant = substr(
                strtolower($this->input),
                $pos + strlen("wordpress is")
            );
        } elseif (
            ($pos = strpos(strtolower($this->input), "wordpress")) !== false
        ) {
            $whatIWant = substr(
                strtolower($this->input),
                $pos + strlen("wordpress")
            );
        }
        ...
        $this->filtered_input = trim(
            str_replace($quantity, "", $this->filtered_input)
        );
        $this->filtered_input = trim(
            str_replace("  ", " ", $this->filtered_input)
        );
