...
class Account {
    ...
    function __construct($uuid, $account_uuid, $account_name) {
		...
		$this->node_list = array("open"=> // choice
							array("credit","debit","close"), // only states available		
							array("credit"=> // only state available
								"open"), // only state available
									array("debit"=>
										"open"), // end state
									array("close"=> // choice
										"open")) // end state
		...
		$this->choice->Create($this->account_name, $this->node_list, $this->state);
	...
	function Create($balance) {
		...
		if ($balance == null) {
			...
			if ($this->balance == null) {
				$null_balance = array("amount"=>0, "attribute"=>'scalar', "unit"=>'none');
				$this->balance = $null_balance;
			}
				

		} else {
			if (is_numeric($balance['amount']) and
				...
				throw new Exception("Needs development for cases where != numeric, string, string balance.");
			}
	...
	function Credit(float $credit_amount) {
		...
		if ($this->state == 'open') {
			...
			$this->balance['amount'] = $this->balance['amount'] + $credit_amount;
		...
		return $this->account_name . ' ' . $this->balance['amount'] . ' ' . $this->balance['unit'];
	}
	...
	function loadBalance() {
		...
		$this->balance = $this->json->readVariable(array('account',$this->account_uuid,$this->account_name));
	...
	function saveBalance() {
		...
		$var_path = array('account', $this->account_uuid, $this->account_name);
