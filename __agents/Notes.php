...
class Notes
{
    ...
    function __construct(Thing $thing, $agent_input = null) 
    {
        ...
        $this->agent_name = "notes";
        $this->agent_prefix = 'Agent "Notes" ';
 ...
 //       $this->thing->choice->load('headcode');
        ...
        $this->keywords = array('notes','next', 'accept', 'clear', 'drop','add','new','here','there');
...
//		$this->thing->log('<pre> Agent "Headcode" completed</pre>');
    ...
    function set()
    {
        ...
        $note = new Variables($this->thing, "variables place " . $this->from);

        //$note->setVariable("place_code", $this->place_code);
...
*/
    function getNotes()
    {
        ...
        $findagent_thing = new Findagent($this->thing, 'console');
        $count = count($findagent_thing->thing_report['things']);
        $this->thing->log('Agent "Notes" found ' . count($findagent_thing->thing_report['things']) ." note Things." );
...
//        }
        ...
        if (!$this->is_positive_integer($count))
        {
            ...
            foreach (array_reverse($findagent_thing->thing_report['things']) as $thing_object)
            {
                ...
                var_dump($thing_object['task']);
                $this->notes[] = array("task"=>$thing_object['task'], "created_at"=>$thing_object['created_at']);
            }
        }
        ...
        foreach(array_reverse($this->notes) as $key=>$note) {

            if (!isset($note['refreshed_at'])) {continue;}
...
*/
            $filtered_places[] = array("note"=>$note['task'], 'created_at' => $refreshed_at);


        }

$created_at = array();
foreach ($this->notes as $key => $row)
{
    $created_at[$key] = $row['created_at'];
}
array_multisort($created_at, SORT_DESC, $this->notes);
...
$this->notes = array();
foreach ($this->old_notes as $key =>$row)
{

//var_dump( strtotime($row['refreshed_at']) );
    if ( strtotime($row['created_at']) != false) { 
      $this->notes[] = $row;
    }
}
...
*/
    ...
    function makeTXT()
    {
        if (!isset($this->notes)) {$this->getNotes();}
//var_dump($this->notes);
        //$this->getNotes();

        if (!isset($this->note)) {$txt = "Not here";} else {
...
*/
        ...
        foreach (array_reverse($this->notes) as $key=>$note) {
            ...
            if (isset($note['created_at'])) {
                ...
                $time_stamp = date("h:i",strtotime($note['created_at']));
                $txt .= $time_stamp;
                //$txt .= " " . "  " .str_pad(strtoupper($time_stamp), 15, "", STR_PAD_LEFT);
            }

            $txt .= " " . str_pad($note['task'], 40, " ", STR_PAD_RIGHT);

//            if (isset($note['created_at'])) {
//                $txt .= " " . "  " .str_pad(strtoupper($note['created_at']), 15, "X", STR_PAD_LEFT);
//            }
            $txt .= "\n";
