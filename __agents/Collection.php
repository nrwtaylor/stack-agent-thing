...
*/
    ...
    function init()
    {
        if (!isset($this->collection)) {$this->collection = 1;}
...
//        $this->makePNG();
    ...
    {
        $sms = "COLLETION | ";
        $sms .= $this->web_prefix . "thing/".$this->uuid."/collection.pdf";
        $sms .= " | TEXT QR";
    ...
    function makeMessage()
    {
        ...
        $message .= "Keep on stacking.\n\n<p>" . $this->web_prefix . "thing/$uuid/collection\n \n\n<br> ";
...
/*
    ...
    {
        $this->thing->json->setField("variables");
        $this->thing->json->writeVariable( array($this->agent_name, "card"), $this->card );
    ...
    function getNuuids()
    {
        ...
        $this->thing->json->setField("variables");
        $this->card = $this->thing->json->readVariable( array($this->agent_name, "thing") );
...
*/
    function makeWeb()
    {
        ...
        $this->node_list = array("web"=>array($this->agent_name,"thing"));
...
//            if ($i == 10) {break;} else {$i += 1;} 
    ...
    function makeTXT()
    {
        ...
        $txt = "This is a COLLECTION of THINGS.\n";
        $txt .= "\n";
...
*/
    ...
    {
        $this->thing->log($this->agent_prefix . 'initialized the collection index.', "INFORMATION");
        ...
        foreach(range(0,$this->size) as $k=>$i) {
            foreach(range(0,$this->size) as $k=>$j) {
                if ($i == $j) {continue;}
                if ($this->things[$i] == $this->things[$j]) {
                    $this->things[$i]= false;
...
echo $thing->uuid . "\n";
    $thing->Create(null,"collection", 's/ make thing');
...
*/
	...
	public function readSubject()
    {
        ...
        $uuid_thing->extractUuids($this->subject);
var_dump(count($uuid_thing->uuids));
        $this->thing->associate($uuid_thing->uuids);


        if (count($pieces) == 1) {

            if ($input == $this->agent_name) {
                $this->makeThings();
...
//                $this->size = 4;
        ...
        $keywords = array("collection", "thing");
