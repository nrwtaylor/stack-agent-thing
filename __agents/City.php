...
class City
{
    ...
    function __construct(Thing $thing, $agent_input = null) 
    {
        ...
        $this->keywords = array('city','next', 'accept', 'clear', 'drop','add','new','here','there');

        $this->default_city_name = $this->thing->container['api']['city']['default_city_name'];
        ...
        $this->railway_city = new Variables($this->thing, "variables city " . $this->from);
    ...
    function set()
    {
        ...
        $this->thing->log( $this->agent_prefix .' set ' . $this->city_code . ' and ' . $this->city_name . ".", "INFORMATION" );

        $city = new Variables($this->thing, "variables " . $this->city_code . " " . $this->from);
    ...
    function isCode()
    {
        ...
        foreach (range(1,9999) as $n) {
            ...
            if ($n >= 9999) {
                $this->thing->log("No City code available in zone " . $city_zone .".", "WARNING");
                return;
            }
        }
    }
    ...
    function nextCode() {
        ...
        foreach ($this->cities as $city) {
            ...
            if (($city_code == $city_code_candidate) or ($city_code_candidate == null)) {
                $city_code_candidate = str_pad(rand(100,9999) , 8, "9", STR_PAD_LEFT);
            }
        }
    ...
    function getCity($selector = null)
    {
        foreach ($this->cities as $city) {
            ...
            if (($selector == null) or ($selector == "")) {
                ...
                $this->city = new Variables($this->thing, "variables " . $this->city_code . " " . $this->from);
                return array($this->city_code, $this->city_name);
            }
        ...
        $cities = array("burnaby", "vancouver", "north vancouver", "new westminster");
        foreach($cities as $city_name) {
            if ($city_name == $selector) {
                return array(999999, $selector);
            }
        }
    ...
    function getCities()
    {
        $this->citycode_list = array();
        $this->cityname_list = array();
        $this->cities = array();

        // See if a headcode record exists.
        $findagent_thing = new Findagent($this->thing, 'city');
        $count = count($findagent_thing->thing_report['things']);
        $this->thing->log('Agent "City" found ' . count($findagent_thing->thing_report['things']) ." city Things." );
        ...
        if (!$this->is_positive_integer($count))
        {
            ...
            foreach (array_reverse($findagent_thing->thing_report['things']) as $thing_object)
            {
                ...
                if (isset($variables['city'])) {
                    ...
                    if(isset($variables['city']['city_code'])) {$city_code = $variables['city']['city_code'];}
                    if(isset($variables['city']['city_name'])) {$city_name = $variables['city']['city_name'];}
                    if(isset($variables['city']['refreshed_at'])) {$refreshed_at = $variables['city']['refreshed_at'];}


                    $this->cities[] = array("code"=>$city_code, "name"=>$city_name, "refreshed_at"=>$refreshed_at);
        ...
        foreach(array_reverse($this->cities) as $key=>$city) {
            ...
            if (!isset($city['refreshed_at'])) {continue;}

            $refreshed_at = $city['refreshed_at'];

            if (isset($filtered_cities[$city_name]['refreshed_at'])) {
                if (strtotime($refreshed_at) > strtotime($filtered_cities[$city_name]['refreshed_at'])) {
                    $filtered_cities[$city_name] = array("name"=>$city_name, "code"=>$city_code, 'refreshed_at' => $refreshed_at);
                }
                continue;
            }
            $filtered_cities[$city_name] = array("name"=>$city_name, "code"=>$city_code, 'refreshed_at' => $refreshed_at);
        ...
        if ($handle) {
            ...
            while (($line = fgets($handle)) !== false) {
                ...
                $this->citycode_list[] = $city_code;
                $this->cityname_list[] = $city_name;
                $this->cities[] = array("code"=>$city_code, "name"=>$city_name); 
            }
       ...
       // Indexing not implemented
        ...
        return array($this->citycode_list, $this->cityname_list, $this->cities);
    }
    ...
    private function get($city_code = null)
    {
        // This is a request to get the Place from the Thing
        // and if that doesn't work then from the Stack.
        if ($city_code == null) {$city_code = $this->city_code;}

        $this->city = new Variables($this->thing, "variables " . $city_code . " " . $this->from);
    ...
    function makeCity($city_code = null, $city_name = null)
    {
        ...
        if ($city_code == null) {$city_code = $this->nextCode();}
        ...
        if ( ($ad_hoc != false) ) {
            ...
            $this->thing->log($this->agent_prefix . "was told the City is Useable but we might get kicked out.");
    ...
    function cityTime($input = null)
    {
        ...
        if ($input == null) {$this->city_time = $city_time;}
    ...
    public function extractCities($input = null)
    {
        ...
        if (!isset($this->cities)) {$this->getCities();}
        ...
        return array($this->city_codes, $this->city_names);
    }
    ...
    public function extractCity($input)
    {
        ...
        if ( ( count($city_codes) + count($city_names) ) == 1) {
            if (isset($city_codes[0])) {$this->city_code = $city_codes[0];}
            if (isset($city_names[0])) {$this->city_name = $city_names[0];}

            $this->thing->log( $this->agent_prefix  . 'found a city code (' . $this->city_code . ') in the text.');
            return array($this->city_code, $this->city_name);
        }

        if (count($city_names) == 1) {$this->city_name = $this->city_names[0];}
    ...
    function assertCity($input)
    {
        ...
        if ($city == true) { // refactor
            // Didn't show up on the stack as a city
            // See if it's in the city resource
            $is_city = $this->isCity(ucwords($filtered_input));

            if (!$is_city) {return true;}

            foreach($this->resource->resources[ucwords($filtered_input)] as $id=>$resource) {
    ...
    function makeTXT()
    {
        if (!isset($this->citycode_list)) {$this->getCities();}

        $this->getCities();

        if (!isset($this->city)) {$txt = "Not here";} else {

        $txt = 'These are CITIES for PLACE ' . $this->railway_city->nuuid . '. ';
        }
        $txt .= "\n";
        $txt .= "\n";
        ...
        foreach ($this->cities as $key=>$city) {
            ...
            $txt .= " " . str_pad(strtoupper(trim($city['name'])), 40, " ", STR_PAD_RIGHT);
            $txt .= " " . "  " .str_pad(strtoupper(trim($city['code'])), 5, "X", STR_PAD_LEFT);
            if (isset($city['refreshed_at'])) {
                $txt .= " " . "  " .str_pad(strtoupper($city['refreshed_at']), 15, "X", STR_PAD_LEFT);
            }
            $txt .= "\n";
        }
    ...
    public function makeSMS()
    {
        ...
        if ((!empty($this->city_code))) {
            $sms .= " | " . $this->web_prefix . 'thing/' . $this->uuid . '/city';
        }
    ...
    function makeWeb()
    {
        ...
        $this->node_list = array("city"=>array("translink", "job"));
        // Make buttons
        $this->thing->choice->Create($this->agent_name, $this->node_list, "city");
    ...
    public function makeImage()
    {
        ...
        imagestring($image, 2, $image_width-75, 10, $this->city_code, $textcolor);
	...
	private function Respond()
    {
        ...
        $this->thing_report['help'] = 'This is a City.  The union of a code and a name.';
    ...
    function lastCity()
    {
        ...
        $this->last_city = new Variables($this->thing, "variables city " . $this->from);
        $this->last_city_code = $this->last_city->getVariable('city_code');
        $this->last_city_name = $this->last_city->getVariable('city_name');

        // This doesn't work
        $this->last_refreshed_at = $this->last_city->getVariable('refreshed_at');
        return;
    ...
    public function readSubject()
    {
                ...
                if (strpos(strtolower($piece),$command) !== false) {
                    ...
                    switch($piece) {
                        ...
                        case 'add':
                            ...
                            if (empty($this->city_name)) {$this->city_name = "X";}

                            $this->response = 'Asserted City and found ' . strtoupper($this->city_name) .".";
                            return;
                            // break;
        ...
        if ($this->city_code != null) {
            $this->getCity($this->city_code);
            $this->thing->log('using extracted city_code ' . $this->city_code . ".","INFORMATION");
        ...
        if ($this->city_name != null) {
            ...
            $this->thing->log('using extracted city_name ' . $this->city_name . ".","INFORMATION");
        ...
        if ($this->last_city_code != null) {
            ...
            $this->thing->log('using extracted last_city_code ' . $this->last_city_code . ".","INFORMATION");
            $this->response = "Last city " . $this->last_city_code . " used to retrieve a City.";
            return;
        }
        ...
        $this->thing->log('using default_city_code ' . $this->default_city_code . ".","INFORMATION");

        $this->response = "Made a City called " . $city . ".";
        return;

        if (($this->isData($this->city_name)) or ($this->isData($this->city_code)) ) {
            $this->set();
            return;
        }
