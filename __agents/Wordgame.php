...
//        $test = "6     U+1F604     😄   grinning face with smiling eyes     eye | face | grinning face with smiling eyes | mouth | open | smile";
        ...
        $this->thing->json->writeVariable( array("wordgame", "letters"), $this->search_words );
    ...
    function get()
    {
        ...
        $time_string = $this->thing->json->readVariable( array("wordgame", "refreshed_at") );
        ...
        $this->search_words = $this->thing->json->readVariable( array("wordgame", "letters") );
    }
    ...
    function test()
    {
        $this->test_result = "red";
        $expected = "best words ??????? MEADOW WARMED WORDAGE";
        $test_thing = new Thing(null);
        $wordgame_test_agent = new Wordgame($test_thing, "wordgame wordgame");
        var_dump($wordgame_test_agent->response);
        if(strpos($wordgame_test_agent->response, $expected) !== false) {
            $this->test_result = "green";
        }
    }
...
//var_dump($new_words);
    ...
    function makeWeb()
    {
        ...
        foreach(str_split($this->search_words) as $letter) {
            // Fix dropping a "" at end of foreach :|
            if ($letter == "") {continue;}
