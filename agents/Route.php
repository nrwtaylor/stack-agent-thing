...
class Route extends Agent
{
    ...
    function set()
    {
        $this->thing->Write(["route", "refreshed_at"], $this->thing->time());
    }
    ...
    function cacheRoute($uuid)
    {
        ...
        try {
            ...
            if (
                ...
                if (
                    time() - strtotime($thing_report["thing"]["refresh_at"]) >
                    0
                ) {
                    $expired = true;
                }
...
return true;
    ...
    public function getRoutes()
    {
        $allowed_endpoints = [];
        if (
            file_exists($this->resource_path . $this->allowed_routes_resource)
        ) {
...
//var_dump($ex);
    ...
    public function hasRoute($text = null)
    {
        ...
        foreach ($this->routes as $i => $route_text) {
            $destarred_route_text = trim(
                strtolower(str_replace("*", " ", $route_text))
            );
