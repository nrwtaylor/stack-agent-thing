...
//$this->thing_report['filtered_message'] = $this->filtered_input;
//$this->thing_report['translated_message'] = $this->translated_input;
...
if (!isset($this->thing_report['sms'])) {
    ...
    if (($this->response != "") or ($this->response != null)) {
        $sms .= " | " . $this->response;
    }

    if (isset($this->sms_message)) {$sms = $this->sms_message;}
...
//$this->thing->subject = $this->input;
            $agent = new $agent_namespace_name($this->thing, $agent_input);
...
//echo "Called get Agent " . $agent_class_name . " saying " . $this->input. "\n";
            if ($this->getAgent($agent_class_name, $this->agent_input)) {
