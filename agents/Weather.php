...
class Weather extends Agent {
    ...
    function doWeather($text) {
        ...
        foreach ($ngram_agent->ngrams as $index=>$ngram) {
            ...
            default:
                if (!isset($this->weather_contents)) {$this->getWeather();}
            }
        }
    }
                    ...
                    if (stripos($line, $day) !== false) {
                        foreach ($days as $day) {
                            if (stripos($a[0], $day) !== false) {
                                ...
                                $night = "day";
                                if (stripos($a[0], "night") !== false) {$night = "night";}
                                $this->daily_forecast[$day][$week_index][$night] = $a[1];
                                if ((strtolower($day) == "monday") and ($night == "day")) {
                                    $week_index += 1;
                                }
                            }
                        }
...
if(!isset($this->daily_forecast['sunday'][0]['night'])) {$index = 1;}
        ...
        foreach (array($index) as $week_index) {
            foreach (array("friday", "saturday", "sunday") as $day) {
                foreach (array("day", "night") as $night) {
                    $night_text = "";
                    if ($night == "night") {$night_text = $night;}
                    if (!isset( $this->daily_forecast[$day][$week_index][$night] )) {continue;}
                    $weather_text .= " / ". trim(ucwords($day) . " " . $night_text)  . ". " . $this->daily_forecast[$day][$week_index][$night];
                }
            }
        }
    ...
    private function nextweekendWeather() {
        $this->response = "+7 day forecast is not available. Try WEATHER THIS WEEKEND. ";
    }
