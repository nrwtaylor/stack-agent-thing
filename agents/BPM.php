...
//        $this->bpm = "X";
        $this->node_list = array("bpm"=>
            array("bar", "tick", "bpm"));

        $this->variables_agent = new Variables($this->thing, "variables bpm " . $this->from);
...
$this->bpm = $this->last_bpm;
        if (!isset($this->bpms)) {$this->extractBPMs($input);}


        if (isset($this->bpms[0])) {$this->bpm = $this->bpms[0];}
        return $this->bpm;
    }
    ...
    function hasBPM($text) {
        ...
        if ((isset($this->bpms)) and (count($this->bpms) > 0)) {return true;}
        return false;
    ...
    public function readSubject() {
        ...
        $input= $this->input;
        //var_dump($this->input);
        $strip_words = array("bpm", "beats per minute");
