...
class Modo extends Agent
{
    ...
    function init()
    {
        ...
        $this->keywords = ["modo", "meet-up", "car", "show", "happening"];

        $this->run_time_max = 360; // 5 hours

        $this->variables_agent = new Variables(
            $this->thing,
            "variables " . "modo" . " " . $this->from
        );

        //   $this->getModo();
        $this->thing_report["help"] =
            "Reads the state of the Modo co-op car fleet. Try MODO 554. Or MODO 470 GRANVILLE VANCOUVER.";
    }
    ...
    function getCars($text = null)
    {
        ...
        foreach ($this->cars as $modo_id => $car) {
            foreach ($car["locations"] as $i => $location) {
                ...
                if (
                    strtolower($location["neighbourhood"]) ==
                    strtolower($this->neighbourhood)
                ) {
    ...
    function getCar($text = null)
    {
        ...
        foreach ($this->cars as $modo_id => $car) {
            foreach ($car["locations"] as $i => $location) {
                if (
                    strtolower($location["neighbourhood"]) == strtolower($text)
                ) {
    ...
    function doModo()
    {
        if (!isset($this->cars) or $this->cars == null) {
            $this->getCars();
        }
        ...
        foreach ($this->cars as $modo_id => $car) {
            ...
            foreach ($car["locations"] as $i => $location) {
                ...
                if ($start_time < $x and $end_time > $x) {
                    $available_flag = false;
                } // Not available.
        ...
        $this->response =
            ...
            $total_cars_count .
            " " .
            number_format($utilization * 100, 0) .
            "%. ";
    }
    ...
    {
        if (!isset($this->neighbourhoods)) {
            $this->getNeighbourhoods($text);
        }
        $this->neighbourhood_matches = [];
        ...
        foreach ($this->locations as $i => $location) {
            if (
                strpos($location["Neighbourhood"], strtolower($text)) !== false
            ) {
    ...
    function getLocation($text = null)
    {
        ...
        //$text = "clark";
        if (isset($this->locations[$text])) {
            $this->location = $this->locations[$text];
            return;
        }
    ...
    public function makeSms()
    {
        ...
        if (isset($this->matches) and $this->matches != []) {
            $this->matchesSms();
            return;
        }
    ...
    public function makeMessage()
    {
        ...
        switch (true) {
            ...
            default:
                $message .=
                    " found " . $this->available_cars_count . " events.";
                $car = reset($this->cars);
                $car_html = $this->carString($car);
    ...
    public function readSubject()
    {
        ...
        if (is_numeric($filtered_input)) {
            $this->getCars();
            if (!isset($this->cars[$filtered_input])) {
        ...
        if ($filtered_input != "") {
            ...
            if ($this->isNeighbourhood($filtered_input)) {
                ...
                $this->response .=
                    "Got neighbourhood " . $this->neighbourhood . ".\n";
            ...
            if ($best_match == null) {
                $this->response =
                    'Place "' . $filtered_input . '" could not be found.';
                return;
            }
...
*/
            ...
            $this->response .=
                "Asked Modo about " . $this->search_words . " cars. ";
            return false;
        }
    ...
    function nearestSms()
    {
        ...
        foreach (array_reverse($this->nearest_cars) as $i => $nearest_car) {
            ...
            if ($car_count == 1) {
                $sms .= "link " . $nearest_car["link"] . " ";
            }
            //            $sms .= $this->carString($nearest_car) . " (" . number_format($nearest_car['distance'], 0) . "m)". " / ";
            $sms .=
                $this->carString($nearest_car) .
                "[" .
                $nearest_car["modo_id"] .
                "]" .
                " (" .
                number_format($nearest_car["distance"], 0) .
                "m)" .
                " / ";
        }
    ...
    function matchesSms()
    {
        ...
        foreach ($this->matches as $location_name => $matches) {
            ...
            $r .= $street_address . " - ";
            if (isset($car["locations"]) and count($car["locations"]) > 1) {
                $r .= " / ";
            }
            $r2 = "";
            foreach ($matches as $modo_id => $car) {
                ...
                foreach ($car["locations"] as $i => $location) {
                    ...
                    $start_time_text = $this->timeString(
                        $location["start_time"]
                    );
        ...
        if ($car_count != count($this->matches)) {
            $sms .= $car_count . " shown ";
        }
        $sms .= "| " . $first_link . " " . $r;
    ...
    function nearestWeb()
    {
        ...
        $html .=
            "<p>Best match location is " .
            $this->location_best_match["description"] .
            ". ";
        $html .=
            "Best match coordinates are (" .
            $this->location_best_match["coordinates"][0] .
            ", " .
            $this->location_best_match["coordinates"][1] .
            ").";
        $html .= "<p>";
        $count = 0;
        if (!isset($this->nearest_cars)) {
