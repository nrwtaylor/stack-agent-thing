...
class Tone extends Agent {
     ...
     */
    function makeSMS() {
        $this->node_list = array("tone"=>array("tone"));
...
$flag = false;
if (strtolower($token) == strtolower($semitone)) {$flag = true;}
if (strtolower($token) == '['.strtolower($semitone) .']') {$flag = true;}
...
$regex_string = '^\b([CDEFGAB](?:b|bb)*(?:#|##|sus|maj|min|aug)*[\d\/]*(?:[CDEFGAB](?:b|bb)*(?:#|##|sus|maj|min|aug)*[\d\/]*)*)(?=\s|$)(?! \w)/i^';
...
//var_dump($semitone);
    ...
    public function readSubject() {
        ...
        if (($pos = strpos(strtolower($this->input), "tone")) !== FALSE) {
            $whatIWant = substr(strtolower($this->input), $pos+strlen("tone"));
...
if (!isset($d_flat)) {$d_flat = $c_sharp;}
if (!isset($d_sharp)) {$d_sharp = $e_flat;}
if (!isset($a_sharp)) {$a_sharp = $b_flat;}
if (!isset($g_flat)) {$g_flat = $f_sharp;}
    ...
    function toneTone($freqOfTone = null, $samplesCount = 44100, $amplitude_level = 1.0)
    {
        ...
        for ($n = $this->total_samples; $n < $samplesCount - $mod + $this->total_samples; $n++) {
