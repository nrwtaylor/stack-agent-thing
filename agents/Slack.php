...
if($this->from == "null@stackr.ca") {return;}
		...
		$this->thing->log( '<pre> Agent "Slack" running on Thing ' .  $this->thing->nuuid . '.</pre>' );
...
//var_dump($this->body);
	...
	private function respond()
    {
		...
		if ($this->response_url != false) {
            ...
            $this->thing_report['info'] = 'Agent "Slack" sent a Slack webhook response.';
...
*/
        ...
        $this->thing->json->setField("message1");
        $this->thing->json->writeVariable( array("debug") , "char_postMessage" . " ". $to . " " . $test . " " . $url );
...
*/
        ...
        echo "<br>open<br>";
        $conn->on('open', function(\Ratchet\RFC6455\Messaging\MessageInterface $msg) use ($conn) {
