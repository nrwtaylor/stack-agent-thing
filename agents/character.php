...
//                        $this->sms_message .= " | This is " . $this->name . " Commands: TBD";
	...
	public function readSubject() {
                ...
                $this->thing->log( '<pre> Agent "Character" started to read "' . $this->subject . '"</pre>' );
...
//$this->charactersGet(); // Load users characters into memory
		...
		$matches = array_unique($matches);
                ...
                if ( count($matches) == 1 ) {
	                ...
	                $this->thing->log( '<pre> Agent "Character" $this->name input ' . $this->name . '</pre>' );
                        $this->getCharacter($matches[0]['name']); 
	                $this->thing->log( '<pre> Agent "Character" $this->name output ' . $this->name . '</pre>' );

                }

		if ( count($matches) == 0 ) {

	                $this->thing->log( '<pre> Agent "Character" dud not match any characters</pre>' );
			$this->nullAction();
...
//			$this->getCharacter($this->subject);
			...
			if (count($matches) == 0) {$this->nullAction();
                        $this->thing->log( '<pre> Agent "Character" did not respond to any keywords and there are no character matches.</pre>' );
