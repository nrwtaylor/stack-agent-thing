...
var_dump(isset($this->error) ? $this->error : "No error seen.");
var_dump("initMongo");
    }
...
//$this->collection = true;
try {
var_dump("getMongo call");
        $result = $this->collection->findOne(["_id" => $text]);
var_dump("getMongo result");
var_dump($result);
        } catch (\Throwable $t) {
...
var_dump("getMongo before return");
var_dump($result);
        //        $result = $this->collection->findOne(["_id" => $text]);
        if ($result == null) {
var_dump("true");
