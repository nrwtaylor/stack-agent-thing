...
class Crow extends Agent
{
    ...
    public function init()
    {
        ...
        $this->response .=
            "crow can hear " . implode(" /", $this->signals) . ".\n";
        ...
        $this->thing_report['help'] =
            'This agent pretends very hard to be a crow.';
    }
    ...
    private function getCrow($requested_nuuid = null)
    {
        ...
        if ($requested_nuuid != null) {
            $entity_input = "get crow " . $requested_nuuid;
        } else {
            $entity_input = "get crow";
        }

        if (!isset($this->crow_thing)) {
    ...
    public function get($crow_code = null)
    {
        ...
        if ($this->signal == false or ($this->signal = "")) {
            $this->signal = "X";
        }

        return [$this->place_name, $this->signal];
    }
...
//        $this->makeSMS();
        ...
        $this->whatisthis = [
            'inside nest' => 'A dark place where crows hatch.',
            'nest maintenance' =>
                'The nest needs to be maintained. Otherwise it falls apart.',
            'patrolling' =>
                "Sit on top of something and watch for weird stuff. Define weird.",
...
//        $this->makeTXT();
    ...
    private function getCrows()
    {
        ...
        foreach ($this->crows as $index => $crow_thing) {
            ...
            $thing_status = null;
            if (isset($variables->thing->status)) {
                $thing_status = $variables->thing->status;
            }

            $number_number = null;
            if (isset($variables->number->number)) {
                $thing_status = $variables->number->number;
            }

            $message_agent = null;
            if (isset($variables->message->agent)) {
                $message_agent = $variables->message->agent;
            }

            $message_outcome = null;
            if (isset($variables->message->outcome)) {
                $message_outcome = $variables->message->outcome;
            }

            $place = null;
            if (isset($variables->place)) {
                $this->place = $variables->place;
            }
    ...
    {
        $test_message =
            "<b>CROW " . strtoupper($this->crow_thing->nuuid) . "</b>" . '<br>';
        $test_message .= "<p>";
    ...
    public function makeSMS()
    {
        ...
        $narratives = [
            "predator" => "Crow is Watching for predators.",
        ...
        $this->litany = [
            'inside nest' =>
                'One of your records was displayed, perhaps by yourself.  A Crow arrived and is waiting in the nest.',
            'nest maintenance' =>
                'A record of yours was displayed again, perhaps by yourself.  This Crow is doing some nest maintenance.',
            'patrolling' => $patrolling_litany,
            'foraging' =>
                "This crow is on it's last legs.  It has gone foraging for stack information about you to forget.",
            'midden work' =>
                'One of your records was displayed, perhaps by yourself.  An Crow arrived and is doing midden work.',
            'start' =>
                "Start.  Not normally means that you displayed a record, let's see if we get any more Crow messages.",
        ];
        ...
        // Behaviour
        $this->crow_behaviour = [
            'inside nest' => 'Crow is in the nest.',
        ...
        if (isset($this->place_name)) {
            $place_text = strtoupper($this->place_name);
        }
        $sms .= " | place " . $place_text;

        $signal_text = "X";
        if (isset($this->signal)) {
            $signal_text = strtoupper($this->signal);
        }
    ...
    function doCrow($text)
    {
        ...
        foreach ($ngram_agent->ngrams as $index => $ngram) {
            switch ($ngram) {
                ...
                case "tag crow":
                    $this->response .=
                        "Tagged Crow " . $this->crow_thing->nuuid . ". ";
                    break;
    ...
    function story()
    {
        // The 90s script
        $n =
            'Information is stored as Things. Things are how ' .
            $this->short_name .
            '.';
        $n .=
        ...
        $n .=
            'Which gets you where exactly? A place where this "Crow" is going to be useful.';
        $n .=
