...
class Coordinate
{
 ...
 //       $this->thing->choice->load('headcode');
        ...
        $this->keywords = array('coordinate','next', 'last', 'nearest', 'accept', 'clear', 'drop','add','new','here','there');

        $this->default_coordinate = $this->thing->container['api']['coordinate']['default_coordinate'];
        ...
        //$this->set();
//echo "coordinate " . $this->stringCoordinate($this->coordinate). "";

        if ($this->agent_input == null) {
		    $this->Respond();
        }
//echo "post-respond coordinate " . $this->stringCoordinate($this->coordinate). "";
        if ($this->agent_input != "extract") {
            $this->set();
        }

        $this->thing->log( $this->agent_prefix .' loaded coordinate ' . $this->stringCoordinate($this->coordinate) . "." );
...
if (($this->coordinate == true) and (!is_array($this->coordinate))) {return;}

        //$this->refreshed_at = $this->current_time;
        $coordinate_variable = new Variables($this->thing, "variables coordinate " . $this->from);

//echo "xxx" . $string_coordinate . "xxx";
        ...
        $this->thing->log( $this->agent_prefix .' set ' . $this->stringCoordinate($this->coordinate) . ".", "INFORMATION" );
...
if (is_array($this->last_coordinate)) {echo "lastcoord"; exit();}
...
*/
    ...
    {
        $this->coordinate_list = array();
        $this->coordinates = array();

        // See if a headcode record exists.
        $findagent_thing = new Findagent($this->thing, 'coordinate');
        $count = count($findagent_thing->thing_report['things']);
        $this->thing->log('Agent "Coordinate" found ' . count($findagent_thing->thing_report['things']) ." place Things." );
...
//echo "meep";
            ...
            foreach (array_reverse($findagent_thing->thing_report['things']) as $thing_object)
            {
                ...
                if (isset($variables['coordinate'])) {
                    ...
                    if(isset($variables['coordinate']['coordinate'])) {$coordinate = $variables['coordinate']['coordinate'];}
                    if(isset($variables['coordinate']['refreshed_at'])) {$refreshed_at = $variables['coordinate']['refreshed_at'];}
/*
...
if (!is_array($coordinate)) {$coordinate = $this->arrayCoordinate($coordinate);}
                        ...
                        $this->coordinates[] = array("coordinate"=>$coordinate, "refreshed_at"=>$refreshed_at);
...
//exit();
        ...
        $filtered_coordinates = array();
        foreach(array_reverse($this->coordinates) as $key=>$coordinate) {

            $coordinate = $coordinate['coordinate'];

            if (!isset($coordinate['refreshed_at'])) {continue;}
            ...
            if (isset($filtered_coordinates[$coordinate]['refreshed_at'])) {
                if (strtotime($refreshed_at) > strtotime($filtered_places[$place_name]['refreshed_at'])) {
                    $filtered_coordinates[$coordinate] = array("coordinate"=>$coordinate, 'refreshed_at' => $refreshed_at);
                }
                continue;
            } 
...
//exit();
    //exit();
       // Indexing not implemented
        ...
        return array($this->coordinate_list, $this->coordinates);
    ...
    private function get($coordinate = null)
    {
        ...
        $this->coordinate_variable = new Variables($this->thing, "variables " . $coordinate . " " . $this->from);

        $coordinate = $this->variables_coordinate->getVariable("coordinate");
        $this->coordinate = $this->arrayCoordinate($coordinate);
        $this->refreshed_at = $this->variables_coordinate->getVariable("refreshed_at");
...
*/
        ...
        $this->thing->log('Agent "Coordinate" will make a Coordinate for ' . $this->stringCoordinate($coordinate) . ".");
      ...
      //      $this->place_thing = $this->thing;
        ...
        $this->thing->log('Agent "Coordinate" found a Coordinate and pointed to it.');
    }
    ...
    function coordinateTime($input = null)
    {
        ...
        if ($input == null) {$this->coordinate_time = $coordinate_time;}
...
//        $pattern = "|\((?<X>\d(\.\d*)?),(?<Y>\d(\.\d*)?),(?<Z>\d(\.\d*)?)\)|";
            ...
            $pattern = "|(?<X>\d(\.\d*)?),(?<Y>\d(\.\d*)?)|";
        $pattern = "|(?<X>\d(\.\d*)?),(?<Y>\d(\.\d*)?),(?<Z>\d(\.\d*)?)|";
        $pattern = "|(?<X>\-?\d(\.\d*)?),(?<Y>\-?\d(\.\d*)?),(?<Z>\-?\d(\.\d*)?)|"; // include (-) numbers
        $pattern = "|(?<X>\-?\\+?\d(\.\d*)?),(?<Y>\-?\+?\d(\.\d*)?),(?<Z>\-?\+?\d(\.\d*)?)|"; // include (-) numbers
...
//        $pattern = "|(?<X>\d(\.\d*)?),(?<Y>\d(\.\d*)?),(?<Z>\d(\.\d*)?)|";
        ...
        if(!isset($m['X'][0])) {
            ...
            $x = $m['X'][0];
            if(isset($m['Y'][0])) {$y = $m['Y'][0];}
            if(isset($m['Z'][0])) {$z = $m['Z'][0];}

        // refactor this.

        $coordinate = array($x);
        if(isset($m['Y'][0])) {$coordinate = array($x,$y);}
        if(isset($m['Z'][0])) {$coordinate = array($x,$y,$z);}
...
if (is_array($input)) {$this->coordinate = true; return;}
...
//        $filtered_input = ltrim(strtolower($input), "place");
    ...
    function assertCoordinate($input)
    {
        ...
        if (($pos = strpos(strtolower($input), "coordinate is")) !== FALSE) { 
            $whatIWant = substr(strtolower($input), $pos+strlen("coordinate is")); 
        } elseif (($pos = strpos(strtolower($input), "coordinate")) !== FALSE) { 
            $whatIWant = substr(strtolower($input), $pos+strlen("coordinate")); 
        }
...
//        $this->get();
    ...
    function makeTXT()
    {
        ...
        if (!isset($this->coordinate)) {$txt = "Not here";} else {

        $txt = 'These are COORDINATES. ';
        }
        $txt .= "\n";
//        $txt .= count($this->placecode_list). ' Place codes and names retrieved.';
        ...
        $txt .= " " . str_pad("COORDINATE", 40, " ", STR_PAD_RIGHT);
        $txt .= " " . str_pad("REFRESHED AT", 8, " ", STR_PAD_LEFT);
...
*/
        ...
        foreach ($this->coordinates as $key=>$coordinate) {
            ...
            if (isset($coordinate['refreshed_at'])) {
                ...
                $txt .= " " . "  " .str_pad(strtoupper($coordinate['refreshed_at']), 15, "X", STR_PAD_LEFT);
            }
            $txt .= "\n";
        ...
        $txt .= "\n";
        $txt .= "Last place " . $this->stringCoordinate($this->last_coordinate) . "\n";
...
//exit();
    ...
    function stringCoordinate($coordinate = null)
    {
       ...
       if ($coordinate == null) {$coordinate = $this->coordinate;}

        if (!is_array($coordinate)) {$this->coordinate_string = true; return $this->coordinate_string;}
...
//return;
       ...
       // var_dump($this->coordinates[0]);

//exit();

        $this->last_coordinate_variable = new Variables($this->thing, "variables coordinate " . $this->from);
       // $this->last_coordinate = $this->last_coordinate_variable->getVariable('coordinate');


        $coordinate = $this->last_coordinate_variable->getVariable("coordinate");

        $this->last_coordinate = $this->arrayCoordinate($coordinate);
    

        // This doesn't work
        $this->last_refreshed_at = $this->last_coordinate_variable->getVariable('refreshed_at');
...
//echo $this->stringCoordinate($this->coordinate);
...
//echo "reads" . $this->stringCoordinate($this->coordinate) . "\n";
//echo "--";

            $this->response = 'Asserted coordinate and found ' . $this->stringCoordinate($this->coordinate) .".";
            return;
        }

        if ($this->coordinate) { // Coordinate not provided in string
            $this->lastCoordinate();
            $this->coordinate = $this->last_coordinate;
            $this->refreshed_at = $this->last_refreshed_at;
            $this->response = 'Asserted coordinate and found last ' . $this->stringCoordinate($this->coordinate) .".";
        ...
        //$this->assertCoordinate($this->coordinate);

        //$this->refreshed_at = $this->thing->time();

        if (empty($this->coordinate)) {$this->coordinate = "X";}
...
//var_dump($this->place_name);
    ...
    if ($this->coordinate != null) {
        ...
        $this->thing->log($this->agent_prefix . 'using extracted coordinate ' . $this->stringCoordinate() . ".","INFORMATION");
        $this->response = $this->stringCoordinate() . " used to retrieve a Coordinate.";
    ...
    if ($this->last_coordinate != null) {
        ...
        $this->thing->log($this->agent_prefix . 'using extracted last_coordinate ' . $this->last_coordinate . ".","INFORMATION");
        $this->response = "Last coordinate " . $this->last_coordinate . " used to retrieve a Coordinate.";
...
$this->thing->log($this->agent_prefix . 'using default_coordinate ' . implode(" ",$this->default_coordinate) . ".","INFORMATION");
...
//$this->getPlace($place);
    ...
    if (($this->isData($this->coordinate)) or ($this->isData($this->coordinate)) ) {
        $this->set();
        return;
    }
