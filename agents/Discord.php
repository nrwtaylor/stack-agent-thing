...
class Discord {
    ...
    function __construct(Thing $thing, $agent_input = null)
    {
        ...
        $this->credential_set = $this->thing->container['api']['discord']['credential_set'];


//        $this->app_token = $this->thing->container['api']['microsoft']['app token'];
        $this->client_id = $this->thing->container['api']['discord'][$this->credential_set]['client_id'];
        $this->client_secret = $this->thing->container['api']['discord'][$this->credential_set]['client_secret'];

        $this->token = $this->thing->container['api']['discord'][$this->credential_set]['token'];

        $this->channel_id = $this->thing->container['api']['discord']['channel_id'];


        $this->permissions_integer = $this->thing->container['api']['discord']['permissions_integer'];
;
		...
		$this->thing->log( 'Agent "Discord" running on Thing ' .  $this->thing->nuuid . '.' );
		$this->thing->log( 'Agent "Discord" received this Thing "' .  $this->subject . '".' );
...
exit();
		...
		if ( $this->readSubject() == true) {
			$this->thing_report = array('thing' => $this->thing->thing, 
		        ...
		        $this->thing->log( 'Agent "Discord" completed without sending a message.' );
			return;
		}
		$this->respond();
...
foreach($synonyms_list as $key=>$item) {
    ...
    function eventSet($input = null)
    {
        ...
        $this->thing->json->writeVariable( array("discord") , $input  );
...
//			$this->thing->log("FB message sent");
			...
			$this->thing_report['info'] = '<pre> Agent "Discord" sent a message to ' . $this->from . '.</pre>';
//
//		} else {
//
...
//$message_reply_id = $this->agent_input;
                    ...
                    $names = $this->thing->json->writeVariable( array("discord", "reply_id"), null );
...
//exit();
    ...
    function authorizeMessage()
    {
        ...
        $jsonDataEncoded = "grant_type=client_credentials&client_id=" . $this->app_id . "&client_secret=" . $this->client_secret . "&scope=https%3A%2F%2Fapi.botframework.com%2F.default";
    ...
    function test()
    {
        ...
        $url = "https://discordapp.com/api/v6/channels/" . $this->channel_id . "/messages";
...
// https://docs.microsoft.com/en-us/microsoftteams/platform/concepts/bots/bot-conversations/bots-conversations#sending-replies-to-messages
        ...
        $url = "https://discordapp.com/api/oauth2/authorize?client_id=157730590492196864&scope=bot&permissions=0";
        //Initiate cURL.
        $ch = curl_init($url);
...
//var_dump($result);
                    ...
                    $this->thing->json->setField("variables");
                    $names = $this->thing->json->writeVariable( array("discord", "result"), $result );
                        $time_string = $this->thing->json->time();
                        $this->thing->json->writeVariable( array("discord", "refreshed_at"), $time_string );
