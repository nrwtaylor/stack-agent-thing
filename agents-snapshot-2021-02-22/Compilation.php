...
class Compilation extends Agent
{

    // List is a reserved word for PHP.
    // So recognize calls for List.
    ...
    function init()
    {
        ...
        $this->thing_report["info"] =
            "Compilation is a collection of things. You could call it LIST.";
        $this->thing_report["help"] = htmlspecialchars(
            "Text RULE <text>. Then text COMPILATION."
        );

        $this->rules_compilation = [];
        $this->rule_tag_count = 0;
        $this->rule_message_count = 0;
        $this->unique_count = 0;

        $this->state = "off";
        if (isset($this->thing->container['api']['compilation']['state'])) {
            $this->state = $this->thing->container['api']['compilation']['state'];
        }
        ...
        // Start with 365 days.
        $this->compilation_horizon = 365 * 24 * 60 * 60;
    ...
    public function getRules()
    {
        ...
        if ($count > 0) {
            foreach (
                ...
                if ($age > $this->compilation_horizon) {
                    continue;
                }
        ...
        $this->rules_compilation = $rules_compilation;
        $this->unique_count = count($rules_compilation);
    }
    ...
    function makeWeb()
    {
        ...
        if ($this->state == "on") {
            if (count($this->rules_compilation) > 0) {
                $rules = "<ul>";
                foreach ($this->rules_compilation as $i => $rule) {
                    $link = $rule['url'];
        ...
        if (count($this->rules_compilation) > 0) {
            $web .=
                "The RULE access keys provided allow deletion of specific channel rules. A new link can be generated at any time by texting COMPILATION.<br>";
        }
    ...
    function makeSMS()
    {
        ...
        $link =
            $this->web_prefix . 'thing/' . $this->thing->uuid . '/compilation';
    ...
    {
        $this->thing->choice->Create('channel', $this->node_list, "compilation");
        $choices = $this->thing->choice->makeLinks('compilation');
